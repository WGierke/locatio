 ...
 */
function calculateRouteFromAtoB(points) {
    var waypoints = [];
    for (var i = 0; i < Object.keys(points).length; i += 1) {
        var waypoint = points[i].lat + ',' + points[i].lng;
    ...
    for (var i = 0; i < Object.keys(points).length; i++) {
        routeRequestParams['waypoint' + i] = waypoints[i];
    }
    var router = platform.getEnterpriseRoutingService(), routeRequestParams;
...
function addMarkersToMap(points) {
    ...
    for (var i = 0; i < Object.keys(points).length; i += 1) {
        var marker = new H.map.Marker({lat: points[i].lat, lng: points[i].lng});
